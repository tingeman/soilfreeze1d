...
class UnfrozenWaterPowerFunction(object):
    ...
    theta_uw = alpha * |T - Tf|^(-beta)  , T <= Tstar
    theta_uw = 1                         , T > Tstar
...
class LayeredModel_unfrw_swi(LayeredModel, UnfrozenWaterPowerFunction):
...
class LayeredModel_unfrw_swia(LayeredModel, UnfrozenWaterPowerFunction):
...
class LayeredModel_unfrw_thfr(LayeredModel, UnfrozenWaterPowerFunction):
...
class LayeredModel_stefan_thfr(LayeredModel, UnfrozenWaterStefanSolution):
...
class LayeredModel_stefan_swia(LayeredModel, UnfrozenWaterStefanSolution):
    """Implements a layered model ground, with freezing characteristics, and four soil constituents:
    grains, water, ice and air. The soil water saturation (in thawed state) can be specified (default=1).
...
class ConvergenceCriteria(object):
    ...
    def calc_change(self, u_0, u_1, unfrw_0, unfrw_1, dt_fraction):
        raise NotImplementedError('calc_change not implemented in base class')
...
def plot_surf(fname=None, data=None, time=None, depths=None, annotations=True,
              figsize=(15, 6), cmap=plt.get_cmap('bwr'), node_depths=True, cax=None,
...
def test_FileStorage():
    ...
    for n in np.arange(20):
        data_storage.add(dt * n, np.ones(5) * n + np.array([0, 0.1, 0.2, 0.3, 0.4]),
                         np.ones(5) * n + np.array([0, 0.1, 0.2, 0.3, 0.4]))
...
def test_FD_unfrw(scheme='theta', Nx=100, version='vectorized', fignum=99, theta=1.0, z_max=np.inf, animate=True):
    ...
    # Set up the plotting
    plot_solution = Visualizer_T(Layers, Tmin=-8, Tmax=4, z_max=z_max, fig=fignum)
    plot_solution.initialize(initialTemperature(x), x, 0., name='test FD unfrw')
    ...
    plot_solution.update(initialTemperature(x), x, t[-1])
...
def test_FD_stefan(scheme='theta', Nx=100, fignum=99, theta=1., z_max=np.inf, animate=True):
    ...
    plot_solution.initialize(initialTemperature(x), x, 0., Layers)
